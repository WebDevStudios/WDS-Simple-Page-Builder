...
class WDS_Page_Builder_Options {
	...
	public function __construct() {
		// Set our title
		$this->title = apply_filters( 'wds_page_builder_options_title', esc_html__( 'Page Builder Options', 'wds-simple-page-builder' ) );
	...
	function add_options_page_metabox() {
		...
		$cmb->add_field( array(
			'name' => esc_html__( 'Template Parts Directory', 'wds-simple-page-builder' ),
			'desc' => esc_html__( 'Where the template parts are located in the theme. Default is /parts', 'wds-simple-page-builder' ),
			'id'   => 'parts_dir',
			'type' => 'text_small',
			'default' => 'parts',
		) );
		...
		$cmb->add_field( array(
			'name' => esc_html__( 'Template Parts Prefix', 'wds-simple-page-builder' ),
			'desc' => esc_html__( 'File prefix that identifies template parts. Default is part-', 'wds-simple-page-builder' ),
			'id'   => 'parts_prefix',
			'type' => 'text_small',
			'default' => 'part',
		) );
		...
		$cmb->add_field( array(
			'name' => esc_html__( 'Allowed Post Types', 'wds-simple-page-builder' ),
			'desc' => esc_html__( 'Post types that can use the page builder. Default is Page.', 'wds-simple-page-builder' ),
			'id'   => 'post_types',
			'type' => 'multicheck',
			'default' => 'page',
		...
		$group_field_id = $cmb->add_field( array(
			'name'         => esc_html__( 'Global Template Parts', 'wds-simple-page-builder' ),
			'desc'         => esc_html__( 'These can be used on pages that don\'t have template parts added to them.', 'wds-simple-page-builder' ),
		...
		$layouts_group_id = $cmb->add_field( array(
			'name'         => esc_html__( 'Saved Layouts', 'wds-simple-page-builder' ),
			'desc'         => esc_html__( 'Use saved layouts to enable multiple custom page layouts that can be used on different types of pages or post types. Useful to create default layouts for different post types or for having multiple "global" layouts.', 'wds-simple-page-builder' ),
			'id'           => 'parts_saved_layouts',
			'type'         => 'group',
			'options'      => array(
				'group_title'   => esc_html__( 'Layout {#}', 'wds-simple-page-builder' ),
				'add_button'    => esc_html__( 'Add another layout', 'wds-simple-page-builder' ),
				'remove_button' => esc_html__( 'Remove layout', 'wds-simple-page-builder' ),
				'sortable'      => true
			)
		) );

		$cmb->add_group_field( $layouts_group_id, array(
			'name'         => esc_html__( 'Layout Name', 'wds-simple-page-builder' ),
			'desc'         => esc_html__( 'This should be a unique name used to identify this layout.', 'wds-simple-page-builder' ),
			'id'           => 'layouts_name',
			'type'         => 'text_medium',
		...
		$cmb->add_group_field( $layouts_group_id, array(
			...
			'id'           => 'template_group',
			'type'         => 'select',
			'options'      => array_merge( wds_page_builder_get_parts(), array( 'add_row_text' => esc_html__( 'Add another template part', 'wds-simple-page-builder' ) ) ),
			'default'      => 'none',
			'repeatable'   => true,
		) );
...
function wds_page_builder_get_parts() {
	...
	$parts['none'] = esc_html__( '- No Template Parts -', 'wds-simple-page-builder' );
	...
	if ( empty( $parts ) ) {
		return esc_html__( 'No template parts found', 'wds-simple-page-builder' );
	}
